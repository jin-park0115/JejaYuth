...
- **증상:** CellMember 페이지에서 같은 셀 명단이 비어 있음 (data: [])
- **원인 1:** RLS 정책에서 `users` 테이블을 재귀적으로 조회 → `infinite recursion detected`
  - **해결:** `SECURITY DEFINER` 함수 사용
- **원인 2:** RLS 정책 조건이 “조회 대상 row의 role” 기준으로 작성됨
  - **해결:** “요청자 역할” 기준으로 판단하도록 정책 수정
    - `get_my_role()` / `get_my_cell()` 함수로 요청자 정보 조회
- **원인 3:** Supabase Auth 세션이 없음 (`auth.getUser()`가 `undefined`)
  - **해결:** 로그인 로직을 `supabase.auth.signInWithPassword`로 변경하여 세션 생성
  - **결과:** RLS가 정상 작동하고 같은 셀 명단 조회 가능
